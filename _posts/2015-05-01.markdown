...
<p>Someone accept the use of setters in a private context. For me is a violation of immutability. You can use setters or initialization methods, but only in a private context: you cannot "set" something because a VO should be stateless.</p>

<p>You can design a Value Object that hold references to Entities. But if Entity change, Value changes too, and this is a violation of the quality of immutability.</p>
...
The method withNickname() must respect the side-effect-free rule. Value Objects are immutable. Thus, the method will generate new container of attributes. Thw use of methods with*, make all more expressive. The method will be implemented in this way:
